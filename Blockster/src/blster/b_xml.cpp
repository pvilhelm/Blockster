...
bster::b_program_tree bster::xml_file_to_program_tree(std::string path)
{
    std::ifstream is;
    std::stringstream ss;
    is.open(path);
    ...
    ss << is.rdbuf();
    return xml_str_to_program_tree(ss.str());
}
