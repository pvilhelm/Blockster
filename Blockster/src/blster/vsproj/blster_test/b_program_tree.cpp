...
void bster::b_program_tree::setVectorOfNodes(std::vector<b_node> v_nodes)
{
...
void bster::b_program_tree::makeTaskVectors()
{
	...
	for (auto node : this->v_nodes) {
		...
		if (str_cmp != task_id) {//round trip failed (not an valid int maybe)
			throw new std::runtime_error("Node: " + node.node_id + "has invalid task id " +
	...
	for (int i = 0; i < v_tasks.size(); i++) {
		...
		if (i != v_tasks.size() - 1){
			...
			if (t.task_id+1 != v_tasks[i + 1].task_id) {
				throw new std::runtime_error("Task ids not in n_(k+1) = n_k+1" + 
	...
	for (auto node : this->v_nodes) {
		...
		b_task& task = v_tasks.at(i_task_id);
		task.addNode(std::move(node));
	}
	v_nodes.clear();

	for (int i = 0; i < v_tasks.size(); i++) {
		b_task& task = v_tasks[i];
